...
def get_optimal_value(capacity, weights, values):
    ...
    print "Capacidade da Mochila: - {:.0f}".format(capacity)

    valuePerWeight = sorted([[values[i], weights[i]] for i in range(numItems)], reverse=True)
    print "Valor - Ordem Decrescente: - {:}".format(valuePerWeight)
    print "---------------------------------------------------------------------------------"
    ...
    while capacity > 0 and numItems > 0:
        ...
        if valuePerWeight[idx][1] <= capacity:
            value += valuePerWeight[idx][0]
            capacityTotal += valuePerWeight[idx][1]
        ...
        print "Valor alocado do item: {:}".format(valuePerWeight[idx][0])
        print "Valor acumulado na Mochila: {:}".format(value)
        print "Capacidade alocada do item: {:}".format(valuePerWeight[idx][1])
        print "Capacidade acumulada na Mochila: {:}".format(capacityTotal)
...
t0 = time()
capacity = 50
values  = [3, 1, 8, 7, 1, 6, 1, 11, 6, 2]
weights = [4, 7, 9, 10, 5, 4, 2, 6, 15 , 18]
opt_value = get_optimal_value(capacity, weights, values)
print("Executado em %0.3fs." % (time() - t0))
...
t0 = time()
capacity = 50
values = [100, 110, 78, 28, 10, 15, 30, 52, 80, 12, 63, 54, 86, 47, 69, 47, 90, 85, 47, 33]
weights = [8, 7, 9, 8, 6, 7, 11, 14, 9, 3, 4, 12, 15, 5, 8, 9, 4, 15, 14, 1]
opt_value = get_optimal_value(capacity, weights, values)
print("Executado em %0.3fs." % (time() - t0))
